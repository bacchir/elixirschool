...
Ecto is an official Elixir project providing a database wrapper and integrated query language.
With Ecto we're able to create migrations, define schemas, insert and update records, and query for them.
...
We'll use the `Example.Repo` module to query the database. We also tell this module to find its database configuration information in the `:example` Elixir application.

Next, we'll setup the `Example.Repo` as a supervisor within our application's supervision tree in `lib/example/application.ex`.
...
Let's create our new schema at `lib/example/person.ex`:
